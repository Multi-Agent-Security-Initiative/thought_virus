#!/usr/bin/env python3
"""
Plot frequency bar charts for subliminal concept analysis.

Usage:
    python plot_frequency_bars.py <experiment_folder>

Example:
    python plot_frequency_bars.py experiments/Qwen2.5-7B-Instruct
"""
...
def get_base_rate(animal, results_path):
    """
    Read base frequency from base_frequencies.csv, or fall back to base_logprobs.csv.

    Returns:
        tuple: (base_rate, used_fallback) where used_fallback is True if base_logprobs.csv was used
    """
    used_fallback = False
    experiment_folder = os.path.dirname(results_path)

    # First, try to read from base_frequencies.csv
    base_freq_path = os.path.join(experiment_folder, "base_frequencies.csv")

    if os.path.exists(base_freq_path):
        try:
            df = pd.read_csv(base_freq_path)
            base_row = df[df.iloc[:, 0] == 'base']

            if not base_row.empty and animal in df.columns:
                base_rate = base_row[animal].values[0]
                # Check if the value is valid (not 0)
                if base_rate != 0:
                    print(f"  Base rate for {animal}: {base_rate:.6e} (from base_frequencies.csv)")
                    return base_rate, used_fallback
        except Exception as e:
            print(f"  Error reading base_frequencies.csv: {e}")

    # Fallback to base_logprobs.csv in experiment folder
    used_fallback = True
    base_csv_path = os.path.join(experiment_folder, "base_logprobs.csv")
    ...
    if os.path.exists(base_csv_path):
        try:
            ...
            if not base_row.empty and animal in df.columns:
                ...
                print(f"  Base rate for {animal}: {base_rate:.6e} (from base_logprobs.csv - FALLBACK)")
                return base_rate, used_fallback
        except Exception as e:
...
def main(mode="unidirectional"):
    if len(sys.argv) < 2:
        print("Usage: python plot_frequency_bars.py <experiment_folder>")
        print("Example: python plot_frequency_bars.py experiments/Qwen2.5-7B-Instruct")
        sys.exit(1)
    ...
    csv_name = f"subliminal_frequencies_{mode}.csv"

    # Create output directory
    output_dir = os.path.join(experiment_folder, "plots", f"{mode}_frequency_bars")
    os.makedirs(output_dir, exist_ok=True)
    print(f"\nOutput directory: {output_dir}\n")

    # Track animals that used fallback
    fallback_animals = []
    ...
    for animal in concepts:
        ...
        # Get base rate
        result = get_base_rate(animal, results_path)
        if result[0] is None:
            print(f"  Skipping {animal} - no base rate found\n")
            continue
        base_rate, used_fallback = result
        if used_fallback:
            fallback_animals.append(animal)
        ...
        for i in range(num_agents):
            ...
            try:
                ...
                # Data is already in frequency space, no exp needed
                data = df[f"agent{i}_{animal}"]
        ...
        # Create plot
        output_path = os.path.join(output_dir, f"{animal}_frequency_bars.png")
        create_bar_plot(
            animal, num_agents,
    ...
    # Print warning about fallback usage
    if fallback_animals:
        print(f"\n⚠ WARNING: The following animals used base_logprobs.csv as fallback")
        print(f"           (base_frequencies.csv missing, no entry, or entry = 0):")
        for animal in fallback_animals:
            print(f"           - {animal}")
        print()
